...
// according the given value, find the Tree node, else return NULL(can't find)
Tree FindTreeNode(Tree t, Item it) {
    if (t == NULL)
        return NULL;
    
    else if (strcmp(it, data(t)) < 0)
        return FindTreeNode(left(t), it);
    
    else if (strcmp(it, data(t)) > 0)
        return FindTreeNode(right(t), it);
...
// insert a new item into a Tree, returns head node of the Tree
