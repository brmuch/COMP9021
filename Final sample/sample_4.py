...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    while len(queue) != 0:
        item = queue.pop()
        index = item[-1]
        for i in closest_words[index]:
            if i == word_2:
                if not solution or len(solution[-1]) > len(item):
                    solution.append(item + [i])
            if not solution and i not in item:
                queue.appendleft(item + [i])

    if candidate_ladder in solution:
        return True
    else:
        return False
